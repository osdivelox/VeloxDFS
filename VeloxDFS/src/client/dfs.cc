...
namespace velox {
	...
	bool DFS::rename(std::string src, std::string dst) {
		...
		vector<BlockMetadata> metadata(NUM_NODES);
		memset(&metadata[0], 0, sizeof(BlockMetadata)*NUM_NODES);
		
		int nChunks =(int)fd->blocks.size();

		for (int i = 0; i < nChunks; i++) {
			auto& host = fd->block_hosts[i];
			int which_node = find(nodes.begin(), nodes.end(), host) - nodes.begin();
			if(metadata[which_node].size == 0){
				metadata[which_node].hash_key = fd->hash_keys[i];
				metadata[which_node].file_name = dst;
				metadata[which_node].name = fd->primary_files[i];
				metadata[which_node].replica = fd->replica;
				metadata[which_node].node = fd->block_hosts[i];
				metadata[which_node].seq = fd->primary_sequences[i];
				metadata[which_node].size = 0;
				metadata[which_node].l_node = nodes[(which_node-1+NUM_NODES)%NUM_NODES];
				metadata[which_node].r_node = nodes[(which_node+1+NUM_NODES)%NUM_NODES];
			...
			smetadata.name = fd->blocks[i];
			smetadata.primary_file = metadata[which_node].name;

			metadata[which_node].size += smetadata.size;
			metadata[which_node].chunks.push_back(smetadata);
		}
